...
export default function EducationSection() {
  ...
  const renderMessage = (message: Message) => {
    ...
    // 訊息框樣式依 type 動態設定
    let msgClass = "message mb-3 p-3 rounded-2xl max-w-[80%] break-words decoration-double text-lg font-bold border-4 border-green-100 shadow-lg bg-white/10 text-white rounded-bl-sm";
    if (type === 'user') {
      msgClass += "bg-white/10 text-white ml-auto rounded-br-sm";
    } else if (type === 'bot') {
      msgClass += "bg-white/10 text-white rounded-bl-sm";
    } else if (type === 'system') {
      msgClass += "bg-blue-50 text-white text-center text-lg mx-auto";
    } else if (type === 'error') {
  ...
  return (
    <section ref={sectionRef} className="py-20 px-4 bg-transparent relative overflow-visible">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <h3 className="text-4xl font-bold font-mono text-text-primary">
        ...
        >
          ...
          <div
            ref={chatMessagesRef}
            className="flex-1 p-6 overflow-y-auto bg-gradient-to-b from-gray-50/50 to-white/0 text-lg relative"
          ...
          <div className="p-6 border-t border-gray-200/50 h-100">
            <div className="flex space-x-3 mb-3">
              <input
                ...
                // 使用者文字輸入設定
                className="flex-1 px-4 py-10 h-14 bg-primary text-white text-lg font-bold border border-primary rounded-full outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all disabled:bg-primary/60 disabled:text-white/60 placeholder:text-white/60"
